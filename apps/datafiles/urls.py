...
urlpatterns = patterns('',
    ...
    url(r'^(?P<id>[\d]+)/?$', \
        'datafiles.views.datafile', name="datafile_details"),

    # permissions
    url(r'^(?P<id>[\d]+)/acl/?$', \
        'datafiles.views.acl', name="datafile_acl"),

    # download
    url(r'^(?P<id>[\d]+)/download/', 'datafiles.views.download', name="download"),

    # extract files from the archive, if file is an archive
    url(r'^(?P<id>[\d]+)/extract/', 'datafiles.views.extract', name="extract"),
