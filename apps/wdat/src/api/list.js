...
WDAT.api.VList = function(name, bus, events, categories) {
	...
	if (categories) {
	  for (var i in categories) {
	    ...
	    if (this.events.add) {
	      var b = new WDAT.api.Button('add', this.bus, this.events.add, {name: cat, id: cat});
...
(function() {
  ...
  ELEM_TMPL = '<tr class="list-elem">' +
	...
	WDAT.api.VList.prototype.add = function(element, category, position) {
	  ...
	  if (!element.id) element.id = this.bus.uid();
	  var id = this._toId(element);
		// Create a new representation of the element e
		var elem = $(ELEM_TMPL);
		elem.attr('id', id);
		elem.find('.list-elem-name').first().text(element.name);
		if (element.info) elem.find('.list-elem-info').first().text(element.info);
		if (element.data) elem.find('.list-elem-data').first().append(element.data);
		// add buttons
  ...
  WDAT.api.VList.prototype.addAll = function(elements, category, position) {
	...
	WDAT.api.VList.prototype.update = function(element, category) {
	  ...
	  if (category && this.categories[category]) tab = this.categories[category];
	  else tab = this._list;
	  // get the element and do update
    var e = tab.find('#' + this._toId(element)).first();
    if (element.name) e.find('.list-elem-name').first().text(element.name);
    if (element.info) e.find('.list-elem-info').first().text(element.info);
    if (element.data) e.find('.list-elem-data').empty().append(element.data);
	};
   ...
   */
	...
	WDAT.api.VList.prototype.selectHandler = function() {
      ...
      if (data.id) that.select(data.id);
	  };
	};
   ...
   */
    ...
    var that = this;
    return function(event, data) {
      if (data.id) that.expand(data.id, null, true);
    };
  };
  ...
  WDAT.api.VList.prototype.removeHandler = function() {
    var that = this;
    return function(event, data) {
      if (data.id) that.remove(data.id);
    };
  };
