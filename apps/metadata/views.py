...
def sections(request):
    """ GET: query all category, PUT/POST: create new, copy """
    ...
    def sections_list(request):
        ...
        sections = sections[:params["max_results"]]
        if sections:
            message_type = "object_selected"
        resp_data = {
            "selected": serializers.serialize("json", ensure_ascii=False, sections), #FIXME check values!!
            #"selected": [{"id": s.id, "name": s.name} for s in sections],
            "sections_total": sections_total,
            "sections_selected": len(sections),
            "selected_range": [start_index, start_index + max_results],
        }
        return BasicJSONResponse(resp_data, message_type, request)

    def create_section(request):
        try: # parse incoming JSON
            deserialized = serializers.deserialize("json", request._get_raw_post_data())
        except:
...
def get_section_last_modified(request, obj_id):
